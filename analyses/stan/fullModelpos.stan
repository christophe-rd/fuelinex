...
parameters{
  ...
  vector<lower = 0>[N_spp] sigma_allo;
...
model{	
  ...
  for(i in 1:N_allo){
    target += lognormal_lpdf(agb_allo[i] | log(agb_allo_pred[i]), sigma_allo[spp_allo[i]]);
  }
//   
//   acc1 ~ lognormal(1, 1);
//   awc1 ~ lognormal(1, 1);
...
// 
//   for(i in 1:N){
//   delta1_trt[i] = normal_rng(log(delta1_pred[i]), s_y[spp[i]]);
//   delta2_trt[i] = normal_rng(log(delta2_pred[i]), s_y[spp[i]]);
//   }
// }
