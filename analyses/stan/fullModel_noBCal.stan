...
for(i in 1:N){
  target += normal_lpdf(delta1[i] | delta1_pred[i], sigma_y[spp[i]]);
  target += normal_lpdf(delta2[i] | delta2_pred[i], sigma_y[spp[i]]);
  }
}
...
generated quantities{
  ...
  for(i in 1:N){
  delta1_trt[i] = normal_rng(log(delta1_pred[i]), sigma_y[spp[i]]);
  delta2_trt[i] = normal_rng(log(delta2_pred[i]), sigma_y[spp[i]]);
  }
}
